...
Using `if` statement and primaries, output `Good morning!` if the first positional parameter is less than 12. Output `Good afternoon!` if it is greater than 12 but less than 18. Otherwise, ouput `Good evening!` if it is more than 18. Take care about cases when positional argument is less than 0 and greater than 24 (print `Error!` in this cases).
